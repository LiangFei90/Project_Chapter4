...
class Window():
        def __init__(self):
                ...
                #self.flist.state(['disabled'])
                self.flist.configure(state='disabled')
                #为文本框添加垂直滚动条
        ...
        def MenuSearchFile(self):
                self.flist.delete(0.0,tkinter.END)
                fname=tkinter.simpledialog.askstring('Findfat','请输入要查找的文件名称：')
                if fname!=None:
                        t=threading.Thread(target=self.ScanFileByName,args=(fname,))
                        t.start()
                self.flist.configure(state='disabled')
        #遍历盘符
        def GetDrives(self):
        ...
        #按名称找文件
        def ScanFileByName(self,fname):
                self.flist.configure(state='normal')
                total=0;
                fname=fname.upper()
                        ...
                        for root,dirs,files in os.walk(drive):
                                for fil in files:
                                        try:
                                                ...
                                                l=len(fn)
                                                if l > 50:
                                                        self.progress['text']=fn[:15]+'...'+fn[l-15:l]
                                                else:
                                                        self.progress['text']=fn
                                                if fn.upper().find(fname)>=0:
                                                        total+=1
